...
resource "null_resource" "create_lex_alias" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      # Update slot priorities for QuerySpendingByCategory intent
      echo "Updating slot priorities for QuerySpendingByCategory intent..."
      QUERY_INTENT_ID=$(aws lexv2-models list-intents \
        --bot-id ${self.triggers.bot_id} \
        --bot-version DRAFT \
        --locale-id en_US \
        --query "intentSummaries[?intentName=='QuerySpendingByCategory'].intentId" \
        --output text)
        
      if [[ ! -z "$QUERY_INTENT_ID" ]]; then
        QUERY_INTENT_INFO=$(aws lexv2-models describe-intent \
          --bot-id ${self.triggers.bot_id} \
          --bot-version DRAFT \
          --locale-id en_US \
          --intent-id $QUERY_INTENT_ID)
          
        # Check if slotPriorities exists for this intent
        if ! echo "$QUERY_INTENT_INFO" | jq -e '.slotPriorities' > /dev/null; then
          echo "⚠️ No slot priorities found for QuerySpendingByCategory intent. Setting them now..."
          ...
          if [[ ! -z "$SLOT_ID_CATEGORY" && ! -z "$SLOT_ID_TIMEFRAME" ]]; then
            # Create a temporary file with the intent configuration
            echo "$QUERY_INTENT_INFO" | jq 'del(.creationDateTime, .lastUpdatedDateTime, .version)' > query_intent.json
            
            # Add slot priorities
            jq --arg cat "$SLOT_ID_CATEGORY" --arg tf "$SLOT_ID_TIMEFRAME" \
              '.slotPriorities = [{"priority": 1, "slotId": $cat}, {"priority": 2, "slotId": $tf}]' \
              query_intent.json > updated_query_intent.json
            ...
            aws lexv2-models update-intent \
              ...
              --locale-id en_US \
              --intent-id $QUERY_INTENT_ID \
              --cli-input-json file://updated_query_intent.json
              
            echo "✅ Slot priorities set for QuerySpendingByCategory intent"
          else
            echo "⚠️ Could not find slot IDs for QuerySpendingByCategory intent"
            exit 1
          fi
        else
          echo "✅ Slot priorities already set for QuerySpendingByCategory intent"
        fi
      else
        echo "⚠️ QuerySpendingByCategory intent not found"
        exit 1
      fi
